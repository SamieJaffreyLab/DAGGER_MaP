...
#to run this pipeline you need to edit this python program to include the path to the correct programs on lines 118, 130, 143.
...
def handler():
    ...
    parser.add_argument("-f", "--vect_file", type=str, )  # , required=True)

    parser.set_defaults(bit_file='merged_R1_R2.bit',
                        reference_file='cool6.fasta',
                        transcript='COOLAIR3',
                        size_file='sizer2.tab',
                        vect_file='forgi-vect-ser2.txt')
...
if __name__ == "__main__":
    ...
    with open('sizer2.tab', 'w') as outS, open('forgi-vect-ser2.txt', 'w') as serF:
        ...
        for t, (profile, num_mutations) in enumerate(sorted(unique_bits.items(), key=lambda x: x[1], reverse=True),
            ...
            if seen[ticks_string] == 1:
                ...
                pipe = ["/home/maxim/contrafold/src/contrafold", "predict", "--constraints", constraint_file, '--parens',
                ...
                # dotbracket
                pipe = [  "python", "/home/maxim/smStructure_seq/fold2dotbracketFasta.py", "--input_file", 'folded/' + bit_prefix + '.fold',
                          "--tag", bit_prefix,
                ...
                # rnaconvert
                pipe = ["python", "/home/maxim/forgi/examples/rnaConvert.py", 'folded/' +  bit_prefix + '.db', '-T', 'element_string', '--force', '--to-file', '--filename', 'folded/' +  bit_prefix + '.txt']
